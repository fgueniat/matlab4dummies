...
To delete \emph{remove} the ith element, use the operator "[]":
...
\tipbox{
	Using lists (for instance \mcode{indices = [1,3,4]}) as arguments of matrix is a very powerful methods for altering a matrix or for creating a new one.
\begin{itemize}
...
In \matlab, using "[]" reinitialize and empty a variable. It is named the \emph{empty vector operator}.
